...
\begin{definition} \textcolor{red}{Возможно копия}
...
\begin{definition} \textcolor{red}{Потенциально копия}
...
\begin{example}
	Рассмотрим $\Z[i] = \{a + bi: a, b \in \Z\}$ "--- \textit{гауссовы числа}. Покажем, что $\Z[i]^* = \{\pm1, \pm i\}$. Для этого определим на $\Z[i]$ \textit{норму} $N : \Z[i] \to \N \cup \{0\}$ следующим образом: для всех $a + bi \in \Z[i]$ положим $N(a + bi) := a^2 + b^2$. Заметим, что тогда для всех $z, w \in \Z[i]$ выполнено $N(zw) = N(z)N(w)$. Значит, все обратимые элементы в $\Z[i]$ имеют норму, равную единице, тогда непосредственной проверкой получим, что $\Z[i]^* = \{\pm1, \pm i\}$.
...
\begin{example}
	Рассмотрим $\Z[\omega] = \{a + b\omega: a, b \in \Z\}$, где $\omega$ "--- нетривиальный корень уравнения $z^3 = 1$ ($\omega = -\frac12 + \frac{\sqrt{3}}2i$), "--- \textit{числа Эйзенштейна}. Покажем, что $\Z[\omega]^* = \{\pm1, \pm \omega, \pm(1 + \omega)\}$. Воспользуемся той же нормой, что и в случае $\Z[i]$. Заметим, что тогда для всех $\forall z = a + b\omega \in \Z[\omega]$ выполнено $N(z) = z\overline{z} = a^2 + ab(\omega + \overline\omega) + b^2\omega\overline\omega = a^2 - ab + b^2$. Элементы нормы, равной единице, найдем перебором и получим, что $\Z[\omega]^* = \{\pm1, \pm \omega, \pm(1 + \omega)\}$.
