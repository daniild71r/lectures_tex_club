...
\begin{example}
	...
	\begin{enumerate}
		...
		Пусть изоморфизм $\phi: \Q(\sqrt[4]{2}) \to \Q(\sqrt[4]{2}i)$ переводит $\sqrt[4]{2}$ в $\sqrt[4]{2}i$, и требуется продолжить его на $\Q(\sqrt[4]{2}, i)$. Многочлен $x^2 + 1 \in \Q(\sqrt[4]{2})[x]$, корнем которого является $i$, не меняется под действием изоморфизма $\phi$. Значит, образом элемента $i$ можно объявить один корней этого многочлена, то есть $i$ или $-i$.
		\item Рассмотрим следующий набор расширений:
		\begin{center}
			\begin{tikzcd}[row sep = small, column sep = tiny, font = \normalsize]
		...
		Пусть изоморфизм $\phi: \Q(\sqrt{2}) \to \Q(\sqrt{2})$ переводит $\sqrt{2}$ в $-\sqrt{2}i$, и требуется продолжить его на $\Q(\sqrt[4]{2})$. Многочлен $x^2 - \sqrt{2} \in \Q(\sqrt{2})[x]$, корнем которого является $\sqrt[4]{2}$, под действием изоморфизма $\phi$ переходит в $x^2 + \sqrt{2}$. Значит, образом элемента $\sqrt[4]{2}$ можно объявить корень многочлена $x^2 + \sqrt{2}$, то есть $\sqrt[4]{2}i$ или $-\sqrt[4]{2}i$.
	\end{enumerate}
\end{example}