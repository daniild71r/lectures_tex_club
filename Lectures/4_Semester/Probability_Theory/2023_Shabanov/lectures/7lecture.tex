...
\begin{theorem} (Леви о монотонной сходимости)
	Пусть $\xi$, $\{\xi\}_{n = 1}^\infty$ --- случайные величины и выполнены следующие свойства:
	\begin{enumerate}
...
\begin{theorem}
	Пусть случайный вектор $\xi \in \R^m$ имеет распределение $P_\xi$ с обобщённой плотностью $p(t)$ по $\sigma$-конечной мере $\mu$ на $(\R^m, \B(\R^m))$. Тогда для любой борелевской функции $g \colon \R^m \to \R$ выполнено следующее:
	\[
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Если $g \ge 0$, то рассмотрим последовательность неотрицательных простых функций $\{g_n\}_{n = 1}^\infty$, подходящих к $g$. По определению математического ожидания:
		\[
...
\begin{corollary}
	Пусть $\xi$ --- дискретная случайная величина, сосредоточенная на $X$. Тогда математическое ожидание $g(\xi)$ можно посчитать следующим образом:
	\[
...
\begin{corollary}
	Пусть $\xi$ --- абсолютно непрерывная случайная величина с плотностью $p(x)$. Тогда верна формула:
	\[
		\E g(\xi) = \int_\R g(x)p(x)dx
	\]
\end{corollary}

\begin{corollary}
	Пусть $\xi$ --- случайный вектор из $\R^m$ с плотностью $p(\vv{x})$. Тогда верна формула:
	\[
		\E g(\xi) = \int_{\R^m} g(\vv{x})p(\vv{x})d\vv{x}
	\]
	где $d\vv{x}$ обозначает обычную меру Лебега в $\R^m$.
\end{corollary}

\begin{note}
	В частности, для $\forall B \in \B(\R^m)$ есть формула:
	\[
		P(\xi \in B) = \int_B p(\vv{x})d\vv{x}
	\]
\end{note}

\textcolor{red}{Стоит ли заменить обозначения приращений на классические, с матана?}

\begin{definition}
	Есть $F$ --- функция распределения вероятностной меры $P$, то обозначим $dF(x) := P(dx)$. Если имеется плотность $p(x)$, то $dF(x) = p(x)dx$
\end{definition}
...
\begin{proof}
	...
	Определим $P$ на $\cA$ по конечной аддитивности. Для того, чтобы продлить построенную меру на $\sigma(\cA) = \F$, остаётся проверить счётную аддитивность этой меры на $\cA$. Пусть $C = \bscup_{i = 1}^\infty C_i$, $C_i, C \in \cA$. Нужно показать такое равенство:
	\[
...
\begin{proof}
	\[
		P_{(\xi, \eta)}(B_1 \times B_2) = P((\xi, \eta) \in B_1 \times B_2) = P(\xi \in B_1 \wedge \eta \in B_2) = P_\xi(B_1) \times P_\eta(B_2)
	\]
\end{proof}
...
\begin{proof}
	Воспользуемся формулой о замене переменной и теоремой Фубини:
	\begin{multline*}
		F_{\xi + \eta}(z) = P(\xi + \eta \le z) = \int_{\R^2} \chi\{x + y \le z\} P_{(\xi, \eta)}(dx, dy) =
		\\
