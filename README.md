...
2. Склонируйте репозиторий: ```$ git clone https://github.com/daniild71r/lectures_tex_club```
   ...
   Создайте в нем свою ветку с уникальным названием, и в своей ветке добавьте в `config.json` запись следующего вида: своего предмета. Ветка **ДОЛЖНА** называться в формате ```"branch_name": ["l/5/Subject_Name/2020_Lecturer", ...]```, где: 
   ...
   Укажите в списке всеспредметы, которые планируете обновлять, наш CI будет компилировать их (и только их) заново при получении нового кода от вас.
