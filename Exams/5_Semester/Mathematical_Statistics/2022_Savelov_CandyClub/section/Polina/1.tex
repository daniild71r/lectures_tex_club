...
\begin{proof} ~
	\begin{enumerate}
		...
		\end{gather*}
		\item
  $$E\sum_{i=1}^{m}\left|\xi_n^{(i)}- \xi^{(i)}\right|^p
 = \sum_{i=1}^{m}E\left(\left|\xi_n^{(i)}- \xi^{(i)}\right|^p\right) $$
 Значит, если левая часть стремится к нулю, то $\forall \ i: \ E\left(\left|\xi_n^{(i)}- \xi^{(i)}\right|^p\right) \xrightarrow{n\rightarrow\infty} 0$.

 Если  $\forall \ i E\left(\left|\xi_n^{(i)}- \xi^{(i)}\right|^p\right) \xrightarrow{n\rightarrow\infty} 0$, то и левая часть стремится к 0
...
\end{proof}
\begin{note}
\textit{Обратная импликация в п.4 неверна}. Достаточно взять $\xi,\eta $ - НОРСВ. $\xi_n \xrightarrow{d} \xi $, $\eta_n \xrightarrow{d} \eta $, где  $\xi \overset{d}{=}\eta$. Но $(\xi_n, \eta_n) \overset{d}{\nrightarrow} (\xi,\xi)$, а $(\xi_n, \eta_n) \overset{d}{\rightarrow} (\xi,\eta)$
\end{note}
\begin{proposition}
